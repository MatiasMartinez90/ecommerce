...
async def migrate() -> None:
    ...
    try:
        ...
        await connection.execute("SELECT pg_advisory_lock(hashtext('ecommerce-migrations'))")
        try:
            for path in sorted(MIGRATIONS.glob("*.sql")):
                sql = migration_up(path)
                checksum = hashlib.sha256(sql.encode()).hexdigest()
                existing = await connection.fetchval(
                    "SELECT checksum FROM service_migrations WHERE version = $1", path.name
                )
                if existing:
            ...
            await connection.execute("SELECT pg_advisory_unlock(hashtext('ecommerce-migrations'))")
    finally:
